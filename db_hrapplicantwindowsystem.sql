...
CREATE TABLE InterviewSchedules ( schedule_id INT AUTO_INCREMENT PRIMARY KEY, application_id INT NOT NULL, interviewer_id INT NOT NULL, interview_date DATETIME NOT NULL, location_mode VARCHAR(150) NOT NULL, -- E.g., 'Zoom Link', 'Room 101' created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP, FOREIGN KEY (application_id) REFERENCES Applications(application_id) ON DELETE CASCADE, FOREIGN KEY (interviewer_id) REFERENCES Users(user_id) ON DELETE RESTRICT ); 
...
CREATE TABLE ApplicationStatusHistory ( history_id INT AUTO_INCREMENT PRIMARY KEY, application_id INT NOT NULL, old_status VARCHAR(50), new_status VARCHAR(50) NOT NULL, changed_by INT, -- Nullable if changed automatically by the system or by the applicant changed_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP, FOREIGN KEY (application_id) REFERENCES Applications(application_id) ON DELETE CASCADE, FOREIGN KEY (changed_by) REFERENCES Users(user_id) ON DELETE SET NULL ); 

CREATE TABLE AuditTrail ( audit_id INT AUTO_INCREMENT PRIMARY KEY, user_id INT, -- Nullable to account for non-authenticated actions or system actions action_type VARCHAR(100) NOT NULL, -- e.g., 'UPDATE', 'DELETE', 'DECISION_MADE' table_affected VARCHAR(50) NOT NULL, record_id INT, details TEXT, action_timestamp TIMESTAMP DEFAULT CURRENT_TIMESTAMP, FOREIGN KEY (user_id) REFERENCES Users(user_id) ON DELETE SET NULL ); 
